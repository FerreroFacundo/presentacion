...
var _branchSegments = 5;

Bolt = function ( materials, radius ) {
	...
	this.buildNode = function() {
		...
		while (intSegmentStep < _branchSegments)
		{
			...
			if( transformedRadius < 0.3) transformedRadius = 0.3
            ...
            colors.push( new THREE.Color(0xffffff).setHSV(1.0,1.0,0.5) );
		...
		while (intSegmentStep < _branchSegments)
		{
			...
			var startY = numCurrentPos/this.totalLinks;
			var endY = startY + 1/this.totalLinks;
...
Bolt.prototype = new THREE.Mesh();
Bolt.prototype.constructor = Bolt;
Bolt.prototype.supr = THREE.Mesh.prototype;
